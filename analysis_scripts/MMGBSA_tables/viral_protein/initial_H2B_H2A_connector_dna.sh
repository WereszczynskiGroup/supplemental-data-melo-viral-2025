...
base_dir="../mmgbsa/H2B_H2A_connector_dna"
mkdir -p $base_dir
...
# Loop over 4 runs
for i in {1..4}; do
  cp virus_${dna_type}.prmtop virus_${dna_type}.run${i}.prmtop
  prmtop="virus_${dna_type}.run${i}.prmtop"
  traj="virus_${dna_type}.run${i}.xtc"

  #Make a temporary directory to hold stripped prmtops for this run
  tmpdir="H2B_H2A_connector_dna_prmtops${i}"
  mkdir -p ${tmpdir}

  # Make receptor trajectory
  cpptraj <<_EOF
parm $prmtop
parmstrip !(:305-377,400-491,503-680,704-776,799-890,902-1079)
...
_EOF
  ...
  cpptraj <<_EOF
parm $prmtop
parmstrip !(:1-294,305-377,400-491,503-680,704-776,799-890,902-1079)
...
strip !(:1-294,305-377,400-491,503-680,704-776,799-890,902-1079)
