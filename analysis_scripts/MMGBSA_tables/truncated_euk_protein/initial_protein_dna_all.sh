...
base_dir="../mmgbsa/protein_dna_all"
mkdir -p $base_dir
...
# Loop over 4 runs
for i in {1..4}; do
  cp trunc_euk_${dna_type}.prmtop trunc_euk_${dna_type}.run${i}.prmtop
  prmtop="trunc_euk_${dna_type}.run${i}.prmtop"
  traj="trunc_euk_${dna_type}.run${i}.xtc"

  #Make a temporary directory to hold stripped prmtops for this run
  tmpdir="protein_dna_all_prmtops${i}"
  mkdir -p ${tmpdir}
